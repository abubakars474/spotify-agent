...
async function playSpotify(url, duration = 30) {
  ...
  //
  const playButton =
    page.locator('button[data-testid="play-button"]').first();
  ...
  if (await playButton.count() > 0) {
    ...
    try {
      ...
      await playButton.click({ delay: 150 });
  ...
  console.log(await playButton.count());

  //
  // SAFARI + WEBKIT FALLBACK (MOST IMPORTANT PART)
  //
  if (!clicked) {
    ...
    try {
      ...
      console.log('Keyboard fallback triggered');

    } catch (e) {

      console.log('Keyboard fallback failed');
    }
  }

  //
  // FINAL VERIFY PLAYBACK (IMPORTANT FOR STABILITY)
  //
  ...
  console.log('Playback active:', isPlaying);
  ...
  if (!isPlaying) {
    ...
    try {
      ...
      //await page.keyboard.press('Space');
      console.log('Space fallback used');
